...
class Pop3Client {
  ...
  private sendMultiLineCommand(cmd: string): Promise<string> {
    return new Promise((resolve, reject) => {
      if (!this.socket) return reject(new Error("Not connected"));
  ...
  async login(): Promise<void> {
    ...
    if (!res.startsWith('+OK')) throw new Error(`Login failed: ${res}`);
  }
  ...
  async list(count: number = 10): Promise<any[]> {
    ...
    for (let i = total; i >= start; i--) {
      ...
      } catch (e) { console.error(`Error fetching ${i}:`, e); }
    }
    return results;
  }
...
function getClient() {
  ...
  if (!user || !pass) throw new Error("EMAIL_USER, EMAIL_PASSWORD required in environment");
...
server.tool(
  ...
  { keyword: z.string(), limit: z.number().default(50) },
  async ({ keyword, limit }) => {
    const client = getClient();
    try {
      await client.connect();
      ...
      if (filtered.length === 0) return { content: [{ type: "text", text: "🔍 No matches found." }] };
...
server.tool(
  ...
  async ({ email_id, filename, save_path }) => {
    const client = getClient();
    try {
      await client.connect();
      await client.login();
      ...
      const attachment = email.attachments.find((a: any) => a.filename === filename);
...
server.tool(
  "read_document",
  "Extract text from local document (pptx, docx, xlsx, pdf)",
  { file_path: z.string() },
  async ({ file_path }) => {
    try {
